import Listener from './components/Learner.js';
import Extensions from './components/extenstions.js';
import { filter, credits } from './components/security.js';
import { rgbaToHex, calculateContrastRatio, keyboardFocusable, isDecimal, wait } from './components/utils.js';
import VoiceType from './components/voicetype.js';
import ai_extension from './extras/aiExtension.js';
import { cookie, session } from './components/storage.js';
import { aiVoice } from './extras/aiVoice.js';
var responce, elemRole='', codeTheme='default', bodyHistory;

window.WebServerAI = class extends ai_extension{
    /**
     * @constructor Loads up the WebServerAI module
     * @param {{}} [settings={}]
     * @param {Boolean} [isNull=false] Triggers event even if message is null
     */
    constructor(settings={},isNull=false){
        super();
        session.save('wsa-aivoice-reader',false);
        const x = document.createElement('script');
        x.innerHTML = 'window.speechSynthesis';
        document.body.insertBefore(x,document.querySelectorAll('script')[1]);
        wait(()=>{aiVoice.getVoices()},50);
        this.isNull = isNull;
        session.save('wsa-isnull',isNull);
        this.request(window.location.origin+'/WebServerAI/libs/history.php?action=create');
        this.request(window.location.origin+'/WebServerAI/libs/ai_credits.php?load=true');
        this.request(window.location.origin+'/WebserverAI/libs/ai_saveSettings.php?action=timezone&value='+Intl.DateTimeFormat().resolvedOptions().timeZone);
        //Load up CSS
        ...
        document.head.innerHTML+='<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/XHiddenProjects/WebAnimate@0.0.5/assets/webanimate.css"/>';
        //Continue the program
        this.theme = settings.hasOwnProperty('theme') ? settings.theme : 'light';
        document.documentElement.setAttribute('wsa-theme',this.theme);
        ...
        this.smart = (settings.hasOwnProperty('smart') ? settings.smart : true);
        ...
        this.language = (settings.hasOwnProperty('ui')&&settings.ui.hasOwnProperty('language') ? settings.ui.language : navigator.language);
        cookie.set('lang',this.language,(86400 * 1));
        if(document.querySelector(this.manualBox))
            document.querySelector(this.manualBox).setAttribute('wsa-exclude','');
        if(document.querySelector(this.submitBtn)){
            const btn = document.querySelector(this.submitBtn);
            btn.setAttribute('wsa-exclude','');
            btn.classList.add('wsa-editor-send');
        }
        ...
        this.botInfo;
        this.listedHistory=[];
        this.getHistory = this.request(window.location.origin+'/WebServerAI/libs/history.php?action=recieve', true);
        if(this.getHistory){
            this.getHistory.forEach((e)=>{
                this.listedHistory.push(e.history);
            });
        }
        this.history=(this.listedHistory ? this.listedHistory : []);
        this.listen = new Listener();
        this.checked=false;
        this.responces = '';

        this.voice = new VoiceType();
    }
    ...
    #formatDate(date){
        ...
        return d.toLocaleDateString(this.language,{
            month: '2-digit',
            day: '2-digit',
            year: 'numeric'
        });
    ...
    #getElemRole(){
        document.querySelectorAll('*:not(.wsa, .wsa *)').forEach((e)=>{
            const node = (window.hasOwnProperty('getComputedAccessibleNode') ? window.getComputedAccessibleNode(e) : null);
                if(node){
    ...
    load(){
        if(this.enabled){
            ...
            document.documentElement.lang = this.language.replace(/\-(.*)$/,'');
            this.botInfo = this.request(this.origin+'/WebServerAI/libs/usersInfo.php',true)['AI'];
            this.previewBox = this.request(this.origin+'/WebServerAI/libs/usersInfo.php',true)['previewBox'];
            this.lang = this.request(this.origin+'/WebServerAI/data/languages.json?u='+this.#uniqid(),true)['languages'][this.language];
            this.botName = this.botInfo['Name'];
            document.querySelector('html').setAttribute('wsa-active','');
            document.querySelector('head').innerHTML+='<link rel="stylesheet" wsa-exclude href="'+this.origin+'/WebServerAI/assets/AI/css/webserverai.min.css"/>';
            document.querySelector('head').innerHTML+='<link rel="stylesheet" wsa-exclude href="'+this.origin+'/WebServerAI/assets/AI/css/webserverai-mobile.min.css"/>';
            if(!this.manualBox){
                ...
                chatBox.setAttribute('wsa-exclude','');
                ...
                ${this.lang['dictionary'].title} 
                    <div class="wsa-subtitle"><span class="wsa-version" title="${this.#formatDate(this.botInfo['Updated'])}">v${this.botInfo['Version']} (${this.lang.name})</span>
                    | <span class="wsa-credits" title="${this.lang['dictionary']['ai']['creditsInfo']}">${this.lang['dictionary']['ai']['credits']}: <span class="current${credits.get() <= Math.floor(credits.max()/2) ? ' low' : ''}">${credits.get()}</span>/${credits.max()}</span></div>
                    </h2>
                    <div class="wsa-ui">
                        <div class="wsa-history"></div>
                        <div class="wsa-editor">
                            <div class="wsa-editor-actions wsa-controllers">
                                <button type="button"  ${credits.get()<=0 ? 'disabled="disabled"' : ''} class="wsa-editor-send wsa-btn" title="${this.lang['dictionary'].nav.send}">
                                    <i class="fa-solid fa-share fa-rotate-180"></i>
                                </button>
                            </div>
                            <textarea class="wsa-userinput" ${credits.get()<=0 ? 'disabled="disabled"' : ''} tab-index="1" spellcheck="false" autocomplete="off"></textarea>
                            <div class="wsa-controllers">
                                ...
                                <div class="wsa-btn-group">
                                    ...
                                    <button class="wsa-btn wsa-clearBox" title="${this.lang['dictionary'].nav.clear}"><i class="fa-solid fa-eraser"></i></button>
                                     <button class="wsa-btn wsa-mic wsa-mic-muted" title="${this.lang['dictionary'].nav.mic}"><i class="fa-solid fa-microphone-slash"></i></button>
                ...
                const el = document.querySelector(this.manualBox);
                if(el){
                    el.classList.add('wsa-userinput');
                    el.setAttribute('wsa-exclude','');
                }else{
                    console.error(this.lang['dictionary'].console.chatBoxNotFound);
                    document.write('');
                    return false;
                }
            }
            ...
            // VoiceType
            this.voice.getTranscript({
                toggleBtn: '.wsa .wsa-mic',
                alternatives: 3,
                results: (event)=>{
                    const result = event.results[event.results.length - 1][0].transcript;
                    document.querySelector('.wsa .wsa-userinput').value = result;
                },
                error: (err)=>{
                    console.error(this.lang['dictionary'].console.voiceTypeSpeechError+': '+err.error);
                    return;
                }
            });

            if(this.previewBtn){
                if(document.querySelector(this.previewBtn)){
                    document.querySelector(this.previewBtn).setAttribute('wsa-exclude','');
                    document.querySelector(this.previewBtn).addEventListener('click',()=>{
                        ...
                        let htmlObject = parse.parseFromString(document.documentElement.innerHTML,'text/html');
                            htmlObject.documentElement.querySelectorAll('[wsa-exclude]').forEach((e)=>{e.remove();});
                            previewEditable.document.documentElement.innerHTML = htmlObject.documentElement.innerHTML;
                        });
                    });
                }else{
                    console.error(this.lang['dictionary']['console']['previewBtnNotFound']);
                    document.write('');
                    return false;
                }
            }
            if(this.preview){
                const preview = document.querySelector(this.preview);
                if(preview)
                    preview.setAttribute('wsa-exclude','');
                else{
                    console.error(this.lang['dictionary']['console']['previewNotFound']);
                    document.write('');
                    return false;
                }
            }
            ...
            document.querySelector('.wsa').addEventListener('mouseup',function(){
                ...
                document.querySelector('.wsa-theme-change').addEventListener('click',function(){
                    if(this.getAttribute('wsa-current-theme')==='light'){
                        ...
                        document.documentElement.setAttribute('wsa-theme','dark');
                    }else{
                        this.setAttribute('wsa-current-theme', 'light');
                        this.querySelector('i').className = 'fa-solid fa-sun';
                        document.documentElement.setAttribute('wsa-theme','light');
                    }
                ...
                document.querySelector('.wsa-history-clear').addEventListener('click',()=>{
                        ...
                        this.history = [];
                        this.request(window.location.origin+'/WebServerAI/libs/history.php?action=delete');
                        console.clear();
                        this.cmdID = 0;
                });
            ...
            txtarea.addEventListener('keydown',(e)=>{
                    ...
                    if(k==38){
                        ...
                        txtarea.value = (this.history[inc] ? this.history[inc].replaceAll('&#34;','"').replaceAll('&#39;',"'").replaceAll('\\,',',') : '');
                    }else if(k==40){
                        e.preventDefault();
                        inc-=1;
                        if(inc < 0){
        ...
        window.addEventListener('load',()=>{
            ...
            animationScript.src = 'https://cdn.jsdelivr.net/gh/XHiddenProjects/WebAnimate@0.0.5/assets/webanimate.js';
    ...
    #updateCredits(txt,submitBtn){
        if(credits.get()<=0){
            txt.classList.add('disabled');
            submitBtn.disabled = true;
        }
        const x = setInterval((box=txt)=>{
    ...
    submit(callback){
        if(this.enabled){
            if(credits.get()>0){
                const txt = (!this.manualBox ? document.querySelector('.wsa-ui textarea') : document.querySelector(this.manualBox)),
                    submitbtn = (!this.submitBtn ? document.querySelector('.wsa-ui .wsa-editor-send') : document.querySelector(this.submitBtn));
                txt.addEventListener('keydown', (e)=>{
                    let key = e.keyCode || e.which;
                    if((key===13&&(!this.isNull&&txt.value!==''))||(key===13&&this.isNull)){
                        e.preventDefault();
                        callback(this.#filter(txt.value));
                        setTimeout(()=>{
                            this.#updateCredits(txt,submitbtn);
                            this.#clearTextbox();
                        },500);
                    }
                });
                submitbtn.addEventListener('click',(e)=>{
                    if((!this.isNull&&txt.value!=='')||this.isNull){
                        e.preventDefault();
                        callback(this.#filter(txt.value));
                        setTimeout(()=>{
                            this.#updateCredits(txt,submitbtn);
                ...
                alert(this.lang['dictionary']['ai']['noCredits']+credits.renew());
        }
    }
    /**
     * Clears out textbox
    ...
    #cmds(str){
        let canReturn = true,
            lastID=0;
        if(str.match(/^wsa --clear$/)){
            let matches = str.match(/^wsa --clear$/);
        ...
        if(str.match(/^wsa --preview$/)){
            const match = str.match(/^wsa --preview$/);
            if(match){
                const preview = window.open(this.origin+'/WebServerAI/data/preview.html','_blank','width='+this.previewBox.width+',height='+this.previewBox.height);
                preview.addEventListener('load',()=>{
                   const parse = new DOMParser();
                   let htmlObject = parse.parseFromString(document.documentElement.innerHTML,'text/html');
                    htmlObject.documentElement.querySelectorAll('[wsa-exclude]').forEach((e)=>{e.remove();});
                    preview.document.documentElement.innerHTML = htmlObject.documentElement.innerHTML;
                });
            }
        ...
        if(str.match(/^wsa --help$/)){
            setTimeout(()=>{
                ...
                <b>preview</b> - `+this.lang['dictionary'].cmd.preview+`<br/>
            `,'info');
            document.querySelector('.wsa-userinput').disabled = false;
        },this.botInfo['ResponceTime']);
            canReturn = false;
    ...
    createCmd(user,msg, type="normal", options=[]){
            const ui = (!this.historyStorage ? document.querySelector('.wsa-ui .wsa-history') : document.querySelector(this.historyStorage)),
                addElem = document.createElement('div');
                const tid = [], tname = [];
                let fixStr = '';
                if(ui){
                if(options.hasOwnProperty('taskInfo')){
                    options.taskInfo.forEach((opt)=>{
                        opt.forEach((item)=>{
                            tid.push(item.task_id);
                            tname.push((item.task_name).toLocaleLowerCase());
                        });
                    });
                    for(let i=0;i<tid.length;i++){
                        fixStr+=tid[i] + ' ' + tname[i] + ', ';
                    }
                    msg = msg.replaceAll('{{task_info}}',fixStr.replace(/, ?$/,'').replace(/,([^,]+)$/,' and $1'));
                }
    ...
    addCmd(msg){
       ...
       let rsp = this.request(this.origin+'/WebServerAI/libs/ai_sender.php?wsaai='+encodeURIComponent(msg)),
           res = this.listen.render(rsp, this.preview);
       this.responces = res;
       if(res){
        const tinfo = [];
        setTimeout(()=>{
            res.forEach((res) => {
                if(res.task_info){
                    tinfo.push(res.task_info);
                }
            });
            this.createCmd(this.botName, this.lang['dictionary'].ai.success, 'success',{taskInfo:tinfo});
                document.querySelector('.wsa-userinput').disabled = false;
            },this.botInfo['ResponceTime']);
       }else{
        setTimeout(()=>{
    ...
    send(msg){
        const filterWords = this.botInfo['blockWords'].concat(this.botInfo['blockDomains']);
        msg = filter(msg, filterWords);
        const user = this.request(this.origin+'/WebServerAI/libs/usersInfo.php',true)['User']['label'];
        document.querySelector('.wsa-userinput').disabled = true;
        if(!this.isNull){
            if(msg){
                if(this.#cmds(msg)){
                    this.history.push(msg);
                    this.request(window.location.origin+'/WebServerAI/libs/history.php?action=add&value='+encodeURIComponent(msg));
                    this.createCmd(user,msg)
                    document.querySelector('.wsa-userinput').disabled = false;
                    credits.use();
                    document.querySelector('.wsa .wsa-title .wsa-credits .current').innerText = credits.get();
                }
            }
        }else{
            if(this.#cmds(msg)){
    ...
    /**
     * Generates smartAI recommendations
     */
